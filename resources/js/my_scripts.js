...
function loadStatsPage(){
	...
	for (var i = 2; i<table.rows.length; i++){
		buffScore = table.rows[i].cells[2].innerHTML;
		oppScore = table.rows[i].cells[3].innerHTML;
...
function loadPlayersPage()
                    {
                       ...
                       var menue= document.getElementById("player_selector");
                       var foo = "";
                       for(var i = 0; i<players.length;i++)
                       {
                           var plyr = players[i].name;
                           foo = foo + "<a class = 'dropdown-item' href='#' onclick = 'switchPlayers("+ i +")'>" + plyr +"</a>"
                       }
                       menue.innerHTML=foo;
...
function switchPlayers(playerNum){
   ...
   var calcpass =Math.floor(players[playerNum].pass_yards/players[playerNum].games_played);
   var calcrush =Math.floor(players[playerNum].rushing_yards/players[playerNum].games_played);
   var calcrec =Math.floor(players[playerNum].receiving_yards/players[playerNum].games_played);
   document.getElementById("avg_p_yards").innerHTML= calcpass;
   document.getElementById("avg_r_yards").innerHTML= calcrush;
    document.getElementById("avg_rec_yards").innerHTML= calcrec;}
